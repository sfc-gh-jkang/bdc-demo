...
if [[ "${SKIP_DCM}" == "false" ]]; then
    ...
    #
    # Idempotency: TRUNCATE each target before COPY so re-running deploy-spcs.sh
    # doesn't append duplicate rows.
    echo "    TRUNCATE RAW tables (idempotent re-run safety)..."
    snow sql -c "${C}" -q "
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CAMPAIGNS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CUSTOMERS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.VEHICLES;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.SERVICE_HISTORY;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALLS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALL_TRANSCRIPTS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALL_SCORES;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.APPOINTMENTS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.TEXT_MESSAGES;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.EMAIL_LOGS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALL_AI_ENRICHMENTS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.PIPELINE_STATUS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.DEALERS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.AGENTS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.AGENT_DAILY_METRICS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.TASKS;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALLS_ICEBERG;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALL_TRANSCRIPTS_ICEBERG;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALL_SCORES_ICEBERG;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.CALL_AI_ENRICHMENTS_ICEBERG;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.APPOINTMENTS_ICEBERG;
        TRUNCATE TABLE IF EXISTS ${SPCS_DATABASE}.RAW.AGENT_DAILY_METRICS_ICEBERG;
    " 2>/dev/null || true
