...
import { streamAnalystQuery, type AnalystMetadata } from '@/api/analyst'
...
function MetadataBadge({ meta }: { meta?: AnalystMetadata }) {
...
export default function Analyst() {
  ...
  async function handleSubmit(q?: string) {
    ...
    try {
      const meta = await streamAnalystQuery(
        text,
        (chunk) => {
          setMessages((prev) => {
            const updated = [...prev]
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-4 border-b border-slate-200 bg-white shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Cortex Agent</h2>
        </div>
        <p className="text-sm text-slate-500 mt-0.5">
          Ask questions about your BDC data in plain English — powered by COACHING_AGENT
        </p>
      </div>
        ...
        {messages.length === 0 && !streaming && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-slate-500 text-sm mb-4">
              Ask a question about calls, agents, or dealer performance
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-xl w-full">
              {SAMPLE_QUESTIONS.map((q) => (
                <button
                  ...
                  className="text-left text-xs px-3 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-colors"
                >
                  {q}
                </button>
              ))}
          ...
          <div key={i} className={`flex gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className="max-w-[80%]">
              <div
                ...
                style={msg.role === 'assistant' ? { boxShadow: 'var(--card-shadow)' } : undefined}
              >
                {msg.content ? (
                  msg.role === 'assistant' ? (
              ...
              {msg.role === 'assistant' && msg.content && <MetadataBadge meta={msg.metadata} />}
            </div>
          </div>
        ))}
      ...
      <div className="px-6 py-3 border-t border-slate-200 bg-white shrink-0">
        <div className="flex gap-2 max-w-3xl mx-auto">
          <input
            ...
            placeholder="Ask about your BDC data..."
            disabled={streaming}
            className="flex-1 px-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-white text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
          />
          <button
